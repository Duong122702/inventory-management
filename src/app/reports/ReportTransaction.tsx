...
import { getReportTransactions } from "../../../api/transactionreport.api";
...
export default function ReportTransaction() {
    ...
    const exportToExcel = (data: any[]) => {
      ...
      if (isError || !transactions) {
        ...
        const dailyTotalsArray = Object.entries(transactions.dailyTotals).map(([date, total]) => ({
      ...
      return (
        <div>
            ...
            <div className="my-2 mx-2">
                <button className="bg-blue-700 text-white py-2 px-4 rounded cursor-pointer" onClick={() => exportToExcel(dailyTotalsArray)}>Xuat file excel</button>
            </div>
        </div>
      )
}
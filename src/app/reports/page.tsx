...
const  Report = () => {
    ...
    const [activeComponent, setActiveComponent] = useState<'product' | 'staff' | 'bill' | 'transaction' | 'shift'>('product'); 

    const handleSelectChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
      setActiveComponent(event.target.value as 'product' | 'staff' | 'bill' | 'transaction' | 'shift');
    };
